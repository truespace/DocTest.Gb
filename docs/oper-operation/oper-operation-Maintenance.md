...
<!-- LLM_Image_DESC_20260408_191856
    유형: Screenshot
    내용: Gamebase 운영 콘솔 점검 목록 화면
    구성: 점검 상태, 시간, 사유 등의 컬럼으로 구성된 등록된 점검 내역 리스트를 보여주는 콘솔 관리 화면
...
<!-- LLM_Image_DESC_20260408_191856
    유형: Screenshot
    내용: Gamebase 기본 제공 점검 팝업
    구성: 게임 이용자에게 표시되는 점검 안내 팝업으로 "Maintenance" 제목, "This service is unavailable during maintenance. We'll be back soon!" 메시지, "SHOW DETAIL" 링크로 구성
    Keyword: 운영, Screenshot, Maintenance, 점검 팝업, 사용자 화면
-->
Gamebase에서 기본으로 제공하는 점검 페이지(점검 사유와 점검 시간 표시)
![gamebase_op_img](./image/gamebase_maintenance_02_ko_240105.jpg)
<!-- LLM_Image_DESC_20260408_191856
    유형: Screenshot
    내용: Gamebase 기본 제공 점검 상세 페이지(웹뷰)
    구성: 모바일 화면 형태의 점검 상세 페이지로, 상단에 "점검" 제목과 닫기(X) 버튼, 렌치 아이콘, "점검으로 잠시 서비스 이용이 제한됩니다." 메시지, 점검 시간 범위(시작~종료 시각 GMT+09:00), 하단에 영문 사과 메시지가 표시되는 웹뷰 화면
    Keyword: 운영, Screenshot, Maintenance, 점검 상세 페이지, 웹뷰
-->
...
<!-- LLM_Image_DESC_20260408_191856
    유형: Screenshot
    내용: Gamebase 운영 콘솔 점검 등록 화면
    구성: 점검 등록 폼으로 (1)대상(전체/일부 클라이언트), (2)사유, (3)기간(Timezone, 시작/종료 시각), (4)점검 페이지 유형(Gamebase 제공/사용자 HTML/외부 페이지), (5)노출 메시지 입력 필드와 미리보기/취소/저장 버튼으로 구성
...
<!-- LLM_Image_DESC_20260408_191856
    ...
    구성: 점검 대상 클라이언트 버전 선택 팝업으로, 전체/테스트/심사 중/베타 서비스/서비스/업데이트 권장 상태별 체크박스와 스토어별(App Store, Google Play, ONE store, Windows Custom, Galaxy Store) 버전 목록을 체크박스로 선택할 수 있는 화면. 취소/확인 버튼 포함
    Keyword: 운영, Console, Screenshot, 대상 클라이언트, 점검 대상
-->
...
<!-- LLM_Image_DESC_20260408_191856
    유형: Screenshot
    내용: 점검 등록 - 외부 페이지 설정 화면
    구성: 점검 페이지 유형에서 '외부 페이지'를 선택한 상태의 설정 화면으로, 점검 정보 제공 체크박스, URL 입력 필드, 언어별(English, Japanese, Korean) 노출 메시지 입력 영역, 기본 언어로 자동 번역 버튼, +추가 버튼, 미리보기/취소/저장 버튼으로 구성
    Keyword: 운영, Console, Screenshot, 외부 페이지, 점검 페이지 설정
-->
