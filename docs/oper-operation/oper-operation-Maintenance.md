...
keyword: Console, WebView, ImageNotice
---
...
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: Gamebase 운영 콘솔의 점검 목록 관리 화면
    구성: '점검 목록' 제목 아래 등록 버튼과 점검 이력 테이블이 표시됨. 테이블에는 상태(예약중/점검중/종료/점검해제 등 색상 라벨), 사유, 시스템, 노출 기간, 등록일 등의 컬럼이 있으며, 다수의 점검 등록 내역이 나열됨. 하단에 페이지네이션이 있음
    Keyword: 운영, Console, Screenshot, Maintenance, 점검 목록
-->
...
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: Gamebase SDK 기본 점검 팝업 (클라이언트 화면)
    구성: 앱 내에서 사용자에게 표시되는 점검 안내 팝업 다이얼로그. 'Maintenance' 제목과 'This service is unavailable during maintenance. We'll be back soon!' 메시지, 하단에 'SHOW DETAIL' 링크 버튼이 있는 심플한 카드 형태의 팝업
    Keyword: 운영, Screenshot, Maintenance, 팝업, 클라이언트
-->
...
<!-- LLM_Image_DESC_20260408_185735
    ...
    구성: 모바일 화면 형태의 점검 상세 페이지. 상단에 '점검' 제목과 X 닫기 버튼, 중앙에 렌치 아이콘과 '점검으로 잠시 서비스 이용이 제한됩니다.' 메시지, 점검 시간 정보(시작~종료 시간 및 GMT+09:00 타임존), 하단에 영문 점검 사유 메시지가 표시됨
    Keyword: 운영, Screenshot, Maintenance, 점검 페이지, 웹뷰, 모바일
-->
...
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: Gamebase 운영 콘솔의 점검 등록 화면
    구성: '점검 등록' 제목 아래 번호가 매겨진 입력 항목이 나열됨. (1)대상: 전체 클라이언트/일부 클라이언트 라디오 버튼, (2)사유: 텍스트 입력, (3)기간: Timezone 선택(UTC+09:00)과 시작/종료 시간 설정, (4)점검 페이지: Gamebase 제공 페이지(웹뷰)/사용자 정의 HTML(웹뷰)/외부 페이지 라디오 버튼, (5)노출 메시지: 언어별 메시지 입력과 기본 언어로 자동 번역 버튼. 하단에 미리보기/취소/저장 버튼이 있음
    Keyword: 운영, Console, Screenshot, Register Maintenance, 점검 등록
-->
...
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: 점검 대상 클라이언트 선택 팝업
    구성: '대상 클라이언트' 제목의 모달 팝업. 상단에 전체 체크박스와 상태별 필터(테스트/심사 중/베타 서비스/서비스/업데이트 권장) 체크박스가 있고, 스토어별(App Store, Google Play, ONE store, Windows Custom, Galaxy Store 등) 클라이언트 버전을 테이블 형태로 표시. 각 버전별 체크박스로 점검 대상을 개별 선택 가능. 하단에 취소/확인 버튼이 있음
    Keyword: 운영, Console, Screenshot, 대상 클라이언트, 팝업
-->
...
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: 점검 등록 시 외부 페이지 설정 화면
    구성: 점검 페이지 유형에서 '외부 페이지'가 선택된 상태의 콘솔 설정 화면. 점검 정보 제공 체크박스(외부 페이지 URL 뒤에 점검 정보 전달 옵션), URL 입력 필드, 노출 메시지 섹션(기본 언어로 자동 번역 버튼, English/Japanese/Korean 등 언어별 메시지 입력 텍스트 영역, + 추가 버튼)이 표시됨. 하단에 미리보기/취소/저장 버튼이 있음
    Keyword: 운영, Console, Screenshot, 점검 페이지, 외부 페이지
-->
