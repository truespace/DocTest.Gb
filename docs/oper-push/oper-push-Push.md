...
<!-- LLM_Image_DESC_20260406
    유형: Screenshot
    내용: Gamebase Push - 예약 목록 및 발송 이력 화면
    구성: 상단에 푸시, 통계, 태그, 이벤트키, 인증서, 설정 탭이 있음. 예약 목록 섹션에 예약 번호, 대상, 발송 타입, 메시지 제목, 메시지 내용, 전체 시발송, 전체 시간 발송, 발송 현황, 발송 예시시간 컬럼의 테이블이 있음. 발송 이력 섹션에 유사한 컬럼의 이력 테이블이 배치됨
    Keyword: 푸시, 예약 목록, 발송 이력, 예약 발송, 즉시 발송
-->
...
<!-- LLM_Image_DESC_20260406
    유형: Screenshot
    내용: Gamebase Push - 푸시 상세 정보 화면
    구성: '푸시 상세 정보' 제목이 있고, 발송 타입, 대상(Android, iOS, iOS Sandbox 등), 이벤트 키 등록, 메시지 타입, 메시지 내용 등 상세 발송 정보가 표시됨. 우측에 Android/iOS 단말기 미리보기가 있음. 하단에 삭제 버튼이 배치됨
    Keyword: 푸시 상세, 발송 타입, 메시지 내용, 미리보기
-->
...
<!-- LLM_Image_DESC_20260406
    유형: Screenshot
    내용: Gamebase Push - 푸시 등록 화면
    구성: 발송 타입(즉시/예약/반복), 대상(전체/특정 회원/그룹/태그), 이벤트 키, 대상 국가, 메시지 타입(정보성/홍보성), 메시지 내용(다국어), 글자색, 클릭 액션, 사용자 정의 필드, 알림음, 큰 아이콘, 버튼, 미디어 등 설정 영역이 순서대로 배치됨. 우측에 Android/iOS 단말기 미리보기가 있음. 하단에 취소/저장 버튼이 배치됨
    Keyword: 푸시 등록, 발송 타입, 대상, 메시지, 미리보기
-->
...
<!-- LLM_Image_DESC_20260406
    유형: UI
    내용: Gamebase Push - 이벤트 키 선택 팝업
    구성: '이벤트 키' 제목 아래에 통계 ID, 이벤트 키, 설명, 수집 기간, 등록일시, 상태(수집중 배지), 선택 버튼 컬럼으로 구성된 이벤트 키 목록 테이블이 배치됨. 하단에 닫기 버튼이 있음
    Keyword: 이벤트 키, 선택, 수집중, 통계 ID, 수집 기간
-->
