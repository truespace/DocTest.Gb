...
keyword: Console, Push, Alert, Android, iOS
---
...
![push_01](./image/push_01_kr_240103.png)
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: Gamebase Push 콘솔의 푸시 메인 화면 (예약 목록 및 발송 이력)
    구성: 상단에 푸시/통계/태그/이벤트키/인증서/설정 탭이 있고, '예약 목록' 섹션에 등록 버튼과 예약된 푸시 테이블(예약 번호, 대상, 발송 타입, 메시지 제목, 메시지 내용, 인원 발송, 현지 시간 발송, 발송 현황, 발송 예정시간 컬럼)이 표시됨. 하단에 '발송 이력' 섹션이 있어 최근 30일 동안의 푸시 발송 이력 테이블이 나열됨
    Keyword: Push, Console, Screenshot, 예약 목록, 발송 이력
-->
...
![push_02](./image/push_02_kr_240103.png)
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: Gamebase Push 콘솔의 푸시 상세 정보 화면
    구성: '푸시 상세 정보' 제목 아래, 좌측에 발송 타입, 대상, 이벤트 키 등록, 대상 국가, 메시지 타입, 메시지 내용 등 푸시 등록 정보가 폼 형태로 표시되고, 우측에 Android/iOS 전환이 가능한 모바일 단말기 미리보기가 표시됨. 하단에 메시지 글자색, 큰 아이콘, 버튼, 미디어 등 추가 설정 항목이 나열됨
    Keyword: Push, Console, Screenshot, 푸시 상세 정보, 미리보기
-->
...
![push_03](./image/push_03_kr_240103.png)
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: Gamebase Push 콘솔의 푸시 등록 화면 (전체 설정 폼)
    구성: '푸시 등록' 제목 아래, 좌측에 (1)발송 타입, (2)대상(전체/특정회원/그룹/태그 발송), (3)이벤트 키, (4)대상 국가, (5)메시지 타입(홍보성/정보성), (6)메시지 내용, (7)메시지 글자색 등 번호가 매겨진 설정 항목이 나열됨. 우측에 모바일 단말기 미리보기가 표시되어 실제 푸시 표시 모습을 확인 가능. 하단에 큰 아이콘, 버튼, 미디어(iOS/Android) 설정과 취소/저장 버튼이 있음
    Keyword: Push, Console, Screenshot, Register Push, 푸시 등록
-->
...
![push_4](./image/push_04_kr_240103.png)
<!-- LLM_Image_DESC_20260408_185735
    유형: Screenshot
    내용: Gamebase Push 콘솔의 이벤트 키 선택 팝업
    구성: '이벤트 키' 제목의 모달 팝업으로, 등록된 이벤트 키 목록이 테이블(등록 ID, 이벤트 키, 설명, 수집 기간, 등록일시, 상태, 선택 컬럼)로 표시됨. 상태가 '수집중'(녹색)인 이벤트 키에는 선택 버튼이 활성화되어 있고, '종료'(회색) 상태의 키는 선택 불가. 하단에 페이지네이션과 닫기 버튼이 있음
    Keyword: Push, Console, Screenshot, 이벤트 키, 팝업
-->
...
>
> - [Android > Register Push](../aos-push.md#2-register-push)
> - [iOS > Register Push](../ios-push.md#2-register-push)
> - [Unity > Register Push](../unity-push.md#2-register-push)
