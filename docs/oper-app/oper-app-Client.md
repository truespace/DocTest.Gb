...
<!-- LLM_Image_DESC_20260406
    유형: Screenshot
    내용: Gamebase 콘솔 클라이언트 목록 화면
    구성: Android(Google Play, ONE store, Galaxy Store, Huawei AppGallery, CHINA), iOS(App Store), Web(Web Custom), Windows(Windows Custom), Mac OS별로 등록된 클라이언트 버전 목록이 색상 코딩된 아이콘으로 표시됨. 각 버전은 서비스 상태에 따라 초록, 노랑, 주황 등의 색상으로 구분됨.
    Keyword: 클라이언트 목록, 운영체제별, 버전 관리, 서비스 상태, Gamebase 콘솔
-->
...
<!-- LLM_Image_DESC_20260406
    유형: Screenshot
    내용: Android 클라이언트 등록 다이얼로그
    구성: 스토어, 클라이언트 버전, 서비스 상태(테스트), 서버 주소, 디버그 로그(SDK 설정), 메모 입력 필드가 포함된 등록 폼. 하단에 취소 및 저장 버튼 존재.
    Keyword: 클라이언트 등록, Android, 스토어, 서비스 상태, 서버 주소
-->
#### (1) 스토어
...
<!-- LLM_Image_DESC_20260406
    유형: UI
    내용: Gamebase SDK 기본 제공 팝업 예시 3종
    구성: (1) Update Recommended - 업데이트 권장 팝업(LATER, UPDATE NOW 버튼), (2) Update Required - 업데이트 필수 팝업(UPDATE NOW 버튼), (3) Service Closed - 서비스 종료 팝업(OK 버튼). 각각 서비스 상태별 기본 안내 메시지 표시.
    Keyword: 팝업, 업데이트 권장, 업데이트 필수, 서비스 종료, SDK
-->
...
<!-- LLM_Image_DESC_20260406
    유형: Screenshot
    내용: 업데이트 필수 상태 시 다국어 메시지 설정 화면
    구성: 서비스 상태를 '업데이트 필수'로 선택한 상태에서 노출 메시지를 Korean, English, Japanese, Chinese-Simplified, Chinese-Traditional 등 다국어로 설정하는 화면. 자세히 보기 버튼 추가 옵션과 해당 버튼 클릭 시 연결될 URL 설정 포함.
    Keyword: 업데이트 필수, 다국어 메시지, 클라이언트 설정, 노출 메시지
-->
...
<!-- LLM_Image_DESC_20260406
    유형: Screenshot
    내용: Android 클라이언트 등록 폼 전체 화면 (업데이트 필수 상태)
    구성: 스토어, 클라이언트 버전, 서비스 상태(업데이트 필수), 다국어 노출 메시지(Korean, English, Japanese, Chinese Simplified, Chinese Traditional), 자세히 보기 버튼 추가, 서버 주소, 디버그 로그, 메모, 설치 URL 등 전체 설정 필드가 표시됨.
    Keyword: 클라이언트 등록, 업데이트 필수, 다국어 설정, 전체 폼
-->
