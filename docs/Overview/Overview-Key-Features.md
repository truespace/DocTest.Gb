...
<!-- LLM_Image_DESC_20260406
    유형: Diagram
    내용: Gamebase Analytics 주요 지표 4가지 소개
    구성: 실시간 지표(실시간 동접 현황 그래프, 1분 단위 실시간 매출/PU/ARPU/ARPPU 등), 매출 지표(결제 금액, 유료 이용자에 대한 다양한 분석, 아이템별 판매 지표), 이용자 지표(주요 KPI 지표를 국가별/OS별/스토어별로 정확하고 전문적인 분석), 밸런싱 지표(게임 레벨별 사용자 맞춤 분석, 매출/습득 아이템 등 밸런싱 지표 제공) 4개의 아이콘과 설명으로 구성
    Keyword: Analytics, 실시간 지표, 매출 지표, 이용자 지표, 밸런싱 지표
-->
